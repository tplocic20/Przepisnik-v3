...
class RecipesService {
  ...
  init() async {
    ...
    await _db.child('Categories').child(globals.userState).once().then((element) {
  ...
  Stream<List<Recipe>> get recipeList {
    return _db.child('Recipes').child(globals.userState).onValue.map((element) => parseRecipes(element.snapshot.value));
  }
